...
Ripley's K-function and L-function are used to quantify the aggregation of points on a landscape within a circular radius from a given point. The resulting aggregation is then compared to a null model to determine if there is a significant difference between the observed pattern of points and the null model. The Complete Spatial Randomness (CSR) model is a commonly used null model. The CSR assumes that points are distributed over a study area in a completely random fashion where the probability of a point occurring at any given location is equal across the entire study area (i.e., the points come from a uniform distribution).

Ripley's K-function quantifies the degree of spatial aggregation of points within a circular radius $r$ and then makes a comparison to what we would expect to see given a completely random process (i.e., CSR). As $r$ increases, Ripley's K value $K(r)$ will increase exponentially because the area of a circle grows exponentially. The L-function takes the square root of Ripley's K value to make the relationship between the resultant L-value $L(r)$ and the radius $r$ linear. Using the L-function the null hypothesis CSR follows a line with a slope of 1 and an intercept of 0; such that if $L(r) > r$, there is evidence for spatial aggregation, whereas if $L(r) < r$ there is evidence for regularity.
</span>
