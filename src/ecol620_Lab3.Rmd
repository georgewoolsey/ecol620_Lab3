...
spatstat.core::Kest(ppp.cactus, correction="none") %>% 
  as.data.frame() %>% 
ggplot(data = ., mapping = aes(x = r)) + 
  geom_line(mapping = aes(y = un, color = "un")) + 
  geom_line(mapping = aes(y = theo, color = "theo"), linetype = "dashed") +
  scale_color_manual(
    values = c("red", "black")
    , labels = c(
      latex2exp::TeX("$\\K_{pois}(r)$")
      , latex2exp::TeX("$\\hat{K}_{un}(r)$")
    )
  ) +
  labs(
    x = latex2exp::TeX("$r$")
...
#plot K
plot(Kest(ppp.cactus, correction="none"), legend=T)
#plot L with 1:1 expectation
Lnone <- Lest(ppp.cactus, correction="none")
plot(Lest(ppp.cactus, correction="none"), legend=T)
